...
String calculateNewVersion(String current, VersionBumpType type) {
  ...
  return switch (type) {
    VersionBumpType.major => '${split[0] + 1}.0.0+1',
    VersionBumpType.minor => '${split[0]}.${split[1] + 1}.0+1',
    VersionBumpType.patch => '${split[0]}.${split[1]}.${split[2] + 1}+1',
    VersionBumpType.build => '${split[0]}.${split[1]}.${split[2]}+${split[3] + 1}',
  };
}
...
enum Flavor {
  ...
  static Flavor fromName(String? name) => values.asNameMap()[name] ?? Flavor.dev;

  static Iterable<String> names = values.map((Flavor it) => it.name);
}
